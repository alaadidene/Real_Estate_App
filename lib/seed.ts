...
async function seed() {
  try {
    ...
    if (propertiesCheck.total > 0) {
      console.log(`Database already has ${propertiesCheck.total} properties. Skipping seed.`);
      alert('Database already seeded! Clear data manually from Appwrite console if you want to re-seed.');
      return;
    }
